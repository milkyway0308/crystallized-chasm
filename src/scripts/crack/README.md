...
# Crack Chasm Crystallized | 결정화 캐즘 | 크랙 플랫폼
...
해당 페이지는 크랙 버전 결정화 캐즘 모듈의 카탈로그입니다.
...
## 결정화 캐즘 - 네오-카피(v3.1.2) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/neocopy.user.js) ]<br>
...
## 결정화 캐즘 - 묶음보고서(v1.2.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/majority-report.user.js) ]<br>

결정화 캐즘 묶음보고서는 크랙 v2 이미지 업로드 패널을 쓰기 편하게 만들어줍니다.

추가로, Kaunsera@Home 모델을 채택하여 이미지 감정 분석을 포함합니다.

## 결정화 캐즘 - 계수기 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/counter.user.js) ]<br>

결정화 캐즘 계수기는 채팅 입력 칸 위에 현재 채팅이 진행된 턴 수를 표시해줍니다.<br>

## 결정화 캐즘 - 절대 영도 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/absolutezero.user.js) ]<br>

결정화 캐즘 절대영도는 랭킹 화면에서의 GIF / WEBP 재생을 중지하고, 호버하는 대상에만 적용시킵니다.<br>

## 결정화 캐즘 - 국소지진계 (v2.0.1) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/local-seismometer.user.js) ]<br>

결정화 캐즘 국소지진계는 요약 메모리의 변경을 감지하고 알립니다. <br>

## 결정화 캐즘 - 자명종 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/alarm-clock.user.js) ]<br>

더 이상의 출석 이벤트 놓침은 그만, 자명종 모듈과 함께 개근상을 노려보세요. <br>

## 결정화 캐즘 - 꿈일기 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/dreamdiary.user.js) ]<br>

결정화 캐즘 꿈일기는 유저노트에 목록을 만들어줍니다. <br>

## 결정화 캐즘 - 손소독제 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/sanitizer.user.js) ]<br>

결정화 캐즘 손소독제는 모바일에서 앱 유도 배너 및 최상단 배너를 자동으로 닫아줍니다.<br>

덤으로 손도 더 깔끔해집니다. 과학이죠!

## 결정화 캐즘 - 아코디언 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/accordion.user.js) ]<br>

결정화 캐즘 아코디언은 스토리 및 캐릭터 에디터의 모든 텍스트에이리어 요소에 크기 조절 기능을 추가합니다.<br>
창 늘리는 재미도 약간 추가됩니다.

## 결정화 캐즘 - 동결 건조 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/lyophilization.user.js) ]<br>

결정화 캐즘 동결 건조는 채팅방 히스토리 추출 기능을 제공합니다. <br>


## 결정화 캐즘 - 복사기 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/copycat.user.js) ]<br>

결정화 캐즘 복사기는 메시지 및 프롬프트 창을 길게 누르면 메시지를 복사하는 기능을 추가합니다. <br>

앱에서의 입력 UX 경험을 PC로 일부 가져오거나, 웹에서 앱의 입력 조작감을 사용하고 싶으신 경우에 좋습니다.<br>

## 결정화 캐즘 - 감시견 (v2.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/watchdog.user.js) ]<br>

결정화 캐즘 감시견은 크랙의 알림을 웹 환경에서 감시하고, 표시해줍니다. <br>
PC 환경에서만 검증되었습니다. 설정에서 OS 푸시를 활성화해야 정상 작동합니다. <br>

## 결정화 캐즘 - 수신거부 (v1.0.0) [ [설치](https://github.com/milkyway0308/crystallized-chasm/dist/crack/optout.user.js) ]<br>
...
> ⚠ 뮤트 모듈은 콜리메이션 프로젝트의 일부로 편성되었으며, 현재 v2로 재작성되고 있습니다.

## 결정화 캐즘 - 네뷸라이저 (v1.4.1)<br>
...
> ⚠ 네뷸라이저 모듈은 콜리메이션 프로젝트의 일부로 편성되었으며, 현재 v2로 재작성되고 있습니다.


## 결정화 캐즘 - 차원이동 (v1.5.5) <br>
...
> ⚠ 차원이동 모듈은 콜리메이션 프로젝트의 일부로 편성되었으며, 현재 v2로 재작성되고 있습니다.

## 결정화 캐즘 - 과포화 (v1.7.2)]<br>

결정화 캐즘 과포화는 채팅의 소모 개수를 과거의 슈퍼챗 개수 표기로 변환하여 소모량 및 잔여량을 더욱 가시적으로 보여줍니다.<br>

> ⚠ 과포화 모듈은 콜리메이션 프로젝트의 일부로 편성되었으며, 현재 v2로 재작성되고 있습니다.

## 결정화 캐즘 - 이그나이터 / 점화기 (v1.6.4)<br>

결정화 캐즘 이그나이터는 재작성된 개선된 버너 모듈입니다.

채팅방의 채팅을 외부 LLM에 의탁하여 요약하며, 프롬프트 공유 및 보관이 가능하도록 개선되었습니다.

> ⚠ 이그나이터 모듈은 콜리메이션 프로젝트의 일부로 편성되었으며, 현재 v2로 재작성되고 있습니다.

## 결정화 캐즘 - 붉은 약 (v1.4.4)<br>
...
> ⚠ 붉은약 모듈은 콜리메이션 프로젝트의 일부로 편성되었으며, 현재 v2로 재작성되고 있습니다.
> ⚠ 세션 아래 표시되는 소모량은 세션 소모량이 아닌 캐릭터 소모량입니다.<br>
> ⚠ 6월 크래커 업데이트 이전 소모된 슈퍼챗은 크래커 35개로 환산되어 계산됩니다.


## 결정화 캐즘 - 폴드 (v1.1.2) [ [설치](https://github.com/milkyway0308/crystallized-chasm/raw/refs/heads/main/crack/fold.user.js) ]<br>

길고 긴 채팅창 세션 목록은 이제 안녕, 결정화 캐즘 폴드와 함께 더 짧은 세션 목록을 사용해보세요!<br>
결정화 캐즘 폴드는 세션 목록을 같은 캐릭터 단위로 묶어 세션 목록에 대신 출력해줍니다.<br>

> ⚠ 폴드 모듈은 현재 v2로 재작성되고 있습니다.

# 은퇴 모듈 목록

## 결정화 캐즘 - 버너+ (v1.7.1-FINALE) [ [설치](https://github.com/milkyway0308/crystallized-chasm/raw/refs/heads/main/crack/retire/burner.user.js) ]<br>
...
## 결정화 캐즘 - 카피(v1.1.4-FINALE) [ [설치](https://github.com/milkyway0308/crystallized-chasm/raw/refs/heads/main/crack/retire/copy.user.js) ]<br>
결정화 캐즘 카피는 크랙 캐릭터 제작자를 위한 보조 도구입니다.<br>
네오-카피로 재작성되어 해당 모듈은 유지보수 지원이 종료되었습니다. <br>
코드 참고 용도로만 사용하세요.<br>
