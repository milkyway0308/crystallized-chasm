...
import { Debouncer } from "../../utils/delay-util";
import { Runnable } from "../../utils/generic-types";
...
function setup() {
  ...
  if (document.getElementById("chasm-lyop-extract-raw-copy")) return;
  injectButton("chasm-lyop-extract-raw-copy", createCopySVG(), "로그 복사", () => {
    ...
    CrackSdk.toastify().doToastifyAlert("메시지를 불러오고 있어요.", 3000);
    const amount = parseInt(prompt("최대 몇 개의 메시지를 가져올까요?\n0으로 입력시, 모든 메시지를 가져옵니다.", "0") ?? "");
    ...
    CrackSdk.sessionFetcher()
      .extractLogs(currentId, { max: amount <= 0 ? -1 : amount })
      .then(async (result) => {
        try {
          if (!result.ok) {
            CrackSdk.toastify().doToastifyAlert("메시지를 가져오는 도중 오류가 발생했어요.");
          } else {
            await window.navigator.clipboard.writeText(
              JSON.stringify(
                {
                  messages: result.value.map((it) => it.simplify()),
                },
                undefined,
                2,
              ),
          ...
          CrackSdk.toastify().doToastifyAlert("클립보드로 채팅방 데이터를 복사했어요.");
        } catch (e) {
          CrackSdk.toastify().doToastifyAlert("클립보드로 데이터를 붙여넣는 도중 오류가 발생했어요.");
        }
      })
      .catch((err) => {
        console.error(err);
  ...
  injectButton("chasm-lyop-extract-raw", createDownloadSVG(), "로그 추출", () => {
    const currentId = CrackSdk.path().chatRoom();
    if (!currentId) {
      CrackSdk.toastify().doToastifyAlert("채팅방 ID를 가져오는데에 실패했어요.\n결정화 캐즘 프로젝트 지원 채널에 제보해주세요.", 3000);
      return;
    ...
    CrackSdk.sessionFetcher()
      ...
      .then(async (result) => {
        ...
        link.href = blobUrl;
        link.download = `${CrackSdk.path().chatRoom()}.txt`;
        link.click();
        link.remove();
...
function injectButton(id: string, svg: string, title: string, action: Runnable) {
  ...
  const currentCategory = CrackSdk.pageComponent().sidePanel().getOrCreateCategory("채팅 내역 추출", "나의 크래커");
  currentCategory.appendNormalButton(id, svg, title, action);
}

BrowserInitUtil.init(() => {
  BrowserInitUtil.onPagePrepare(() => {
    const debouncer = new Debouncer(setup);
